...
public class MapleMonster extends AbstractLoadedMapleLife {
    ...
    private final void giveExpToCharacter(final MapleCharacter attacker, int exp, final boolean highestDamage, final byte pty, final int lastskillID) {
        final int trueExpRate = RateManager.getTrueExpByLevel(attacker.getLevel());
        if (eventDrop > 0) {
            return;
        }
        if (highestDamage) {
        ...
        if (exp > 0) {
            ...
            if (ms != null) {
                ...
                exp += (int) (rawexp * trueExpRate * (skillEffect.getX() / 100.0));  
            }
            ...
            if (holySymbol != null) {
                ...
                if (canFullHSEffect) { //파티원수 인원 체크여부
                    exp += rawexp * trueExpRate * (holySymbol.doubleValue() / 100.0);  
                } else {
                    exp += rawexp * trueExpRate * (holySymbol.doubleValue() / 100.0);  
                }
            ...
            double expBuff = rawexp * trueExpRate * (attacker.getStat().expBuff - 100) / 100.0;  
